...
class YoloDetector:
    def __init__(
        ...
        """
        self._class_path = pathlib.Path(
            __file__
        ).parent.absolute()  # os.path.dirname(inspect.getfile(self.__class__))
        self.gpu = gpu
        ...
        if self.frontal:
            self.anti_profile = joblib.load(
                os.path.join(
                    self._class_path, "models/anti_profile/anti_profile_xgb_new.pkl"
                )
            )
    ...
    def _postprocess(self, imgs, origimgs, pred, conf_thres, iou_thres):
        ...
        for i in range(len(origimgs)):
            ...
            det = pred[i].cpu()
            scaled_bboxes = scale_coords(
                imgs[i].shape[1:], det[:, :4], img_shape
            ).round()
            scaled_cords = scale_coords_landmarks(
                imgs[i].shape[1:], det[:, 5:15], img_shape
            ).round()
            ...
            for j in range(det.size()[0]):
                ...
                lm = list(
                    map(int, [i * w if j % 2 == 0 else i * h for j, i in enumerate(lm)])
                )
